...
const eventMatchesBinding = (e: KeyboardEvent, binding: string): boolean => {
  ...
  // Reject unrequested modifiers too, e.g. Ctrl+Tab must not match plain "Tab"
  if (!needMod && !needCtrlOnly && e.ctrlKey) return false;
  if (!needMod && !needMetaOnly && e.metaKey) return false;
