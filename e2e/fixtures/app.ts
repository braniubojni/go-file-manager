...
export async function selectRow(page: Page, id: Pane, name: string) {
  ...
  await grid.click()
  const row = grid.locator('.MuiDataGrid-row').filter({ hasText: rowFilter(name) }).first()
...
async function ensureExtensionsVisible(page: Page) {
  ...
  await dismissMenus(page)
  await expect(grid.locator('.MuiDataGrid-row').filter({ hasText: 'note.txt' }).first()).toBeVisible()
}
